 ...
 * it will display the available ports -f "filepath1": send file (filepath
...
int main (int argc, [[maybe_unused]] char* argv[])
{
    ...
    cliParser.setCommands (
        { { "-h",
            ...
            {
                logger->info (
                    "This program allows you to send one or more files "
          ...
          { "-f",
            [&files, &logger] (std::string_view value)
            {
                logger->info (value);
                files.emplace_back (
                    std::make_shared<DesktopFile> (value.data()));
            } },
