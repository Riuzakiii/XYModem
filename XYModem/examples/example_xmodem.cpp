...
int main (int argc, [[maybe_unused]] char* argv[])
{
    ...
    XModemSender xmodem (serialHandler, logger);
    ...
    cliParser.setCommands({
        ...
        {"-f", [&filePath, &logger](std::string_view value){
                logger->info (value);
                filePath=value.data();
        }},
    ...
    try
    {
        ...
        {
            xmodem.transmit (
                std::make_shared<xymodem::DesktopFile>(filePath), [] (float) {}, [] () { return false; }, withHex);
        }
    }
